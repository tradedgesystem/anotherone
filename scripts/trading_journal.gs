...
function createTradingJournal() {
  ...
  tradesSheet.getRange('A1:D1').setValues([
    ['Date', 'Trade Slot', 'Strategy Used', 'Liquidity Level'],
  ]);
  ...
  tradesSheet.insertRowsAfter(1, maxRows - tradesSheet.getMaxRows());
  ...
  tradesSheet.getRange('A1:D1').setFontWeight('bold');
  tradesSheet.setFrozenRows(1);
  tradesSheet.setColumnWidths(1, 4, 180);
